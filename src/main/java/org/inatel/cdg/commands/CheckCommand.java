...
public class CheckCommand implements Command
{
    ...
    private EmbedBuilder makeEmbed(CommandContext ctx, PetEntity pet)
    {
        ...
        embed.addField("📛 Name", pet.getPetName(), true);
        embed.addField("⚡ Energy", pet.getPetEnergy() + "/100", true);
        embed.addField("🍕 Hunger", pet.getPetHunger() + "/100", true);
        embed.addField("💰 Money", pet.getPetBalance() + "", true);
        embed.addField(
                "💤 Status",
                pet.getSleeping() ? "Sleeping 😴" : "Awake 😄",
                true
        );
